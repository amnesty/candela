module ActiveRecord #:nodoc:
  ...
  #
  # == Named Scope
  # You can use the scope +in_container+ to get all Contents in some Container.
  ...
  module Content
    class << self
      def included(base) # :nodoc:
        ...
        base.scope :in_container, lambda { |container| {} }
...
#          attr_protected reflections[options[:reflection]].foreign_key
#          if reflections[options[:reflection]].options[:polymorphic]
#            attr_protected reflections[options[:reflection]].foreign_type
#          end
        end
#        attr_protected :container, :container_id, :container_type

        scope :in_container, lambda { |container|
      ...
      def container_conditions(container)
        ...
        when Array
          ...
          if container.class.acts_as?(:container)
            c = "#{ table_name }.#{ container_reflection.foreign_key } = '#{ container.id }'"
            if container_reflection.options[:polymorphic]
              c << " AND #{ table_name }.#{ container_reflection.foreign_type } = '#{ container.class.base_class }'"
            end
            "(#{ c })"
          else
            ""
