      ...
      def sortable_list(list, list_class, options = {})
        ...
        "".tap do |html|
          ...
          for object in list
            ...
            # Show
            html << link_to(image_tag("icons/show.png", :alt => t(:show), :title => t(:show)), object) if actions.delete(:show)

            # Delete
            delete_html = link_to(image_tag("icons/delete.png", :alt => t(:delete), :title => t(:delete)), object, :confirm => t(:confirm_delete, :scope => object.class.to_s.underscore), :method => :delete) if actions.delete(:delete)

            # Rest of actions
            actions.each do |a|
              html << link_to(image_tag("icons/#{ a }.png", :alt => t(a), :title => t(a)), polymorphic_path(object, :action => a))
            end
