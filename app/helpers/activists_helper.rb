module ActivistsHelper
  ...
  def collaboration_row(activist, collaboration)
    ...
    text = t( "activists_collaboration.texts.#{ i18n_key }", 
              :collaboration_type => link_to_if( collaboration.authorize?(:read, :to => current_agent), h(collaboration.kind), activist_activists_collaboration_path(collaboration.activist, collaboration) ),
    ...
    collaboration.is_leave ? content_tag(:strike, text.html_safe) : text.html_safe
  end
end
