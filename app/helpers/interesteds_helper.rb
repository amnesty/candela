module InterestedsHelper
  ...
  def previous_history_events_for(interested)
    ...
    events << {:date => interested.created_at.to_date, :info => t('interested.previous_history_events.minor_checked')} if interested.minor_checked
    ...
    events << {:date => interested.activist.created_at.to_date, :info => link_to(t('interested.previous_history_events.migrated'), interested.activist )} if interested.activist.present?

    events.sort_by{|e|e[:date]}
    #events.collect{|item| content_tag :li, "#{l item[:date]}: #{item[:info]}".html_safe}.join.html_safe
  end
