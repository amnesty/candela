  ...
  def interested_show_action_buttons(interested)
    ''.tap do |html|
        if interested.activist.present?
          ...
          html << "<ul>"
          html << object_action_button_to(interested, 'prepare_mail', :wrapper_tag => :li) if current_user.has_any_permission_to(:create_email, :interesteds, :on => Site.current)
          html << object_action_button_to(interested, 'prepare_pdf', :wrapper_tag => :li) if current_user.has_any_permission_to(:create_pdf, :interesteds, :on => Site.current)
