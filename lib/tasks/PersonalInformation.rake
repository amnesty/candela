...
namespace :personal_information do
  namespace :clean do
    ...
    task :sensitive_information => :environment do
      ...
      activists.each do |activist|
        ...
        if !activist.leave_at.nil? 
          if activist.mobile_phone == '999999999'
            ...
            activist.save_without_validation
          else
            puts "Activist ##{ activist.id } -> Not cleaning yet (leave at #{activist.leave_at})"
          end
        # Set leave to activist with no collaborations or no actvie collaborations and time has exceed
      ...
      puts "Getting interested with created_at date minor than #{ I18n.localize(remove_time.to_date) }"
